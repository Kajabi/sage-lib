...
You'll need to wrap sections expected to collapse and show the button in `<ToolbarEditor />`s inside of a `flexbox` container to receive the desired visual treatment.
Sections that are not intended to have the responsive behavior can be wrapped in the class `toolbar-editor__button-group`.
